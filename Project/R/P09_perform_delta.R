...
# stylo.perform.delta bug fixed by P09 on existing stylo library, that select nearest neighbour
perform.delta_P09_ALDA = function(training.set, 
                         test.set,
  ...
  for(h in 1:length(selected.dist[,1])) {
    ...
    #Begin P09 JuanJoseRivera Modification
    freq_table<-table(current.ranking)
    current.sample<-names(freq_table)[which (freq_table==max(freq_table))][1]
    classification.results = c(classification.results, current.sample)
    #End P09 JuanJoseRivera Modification
